...
export default function AcceptInvitePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:3000/api/auth/accept-invite", {
        method: "POST",
    ...
    <div className="min-h-screen flex flex-col" style={{ fontFamily: '"SimStd", sans-serif' }}>
      ...
      <div className="flex flex-1 items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-md bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          <div className="mb-8">
            <h1 className="text-xl font-bold text-black/70">Passwort festlegen</h1>
            <p className="text-sm text-black/40 mt-1">Bitte legen Sie ein Passwort für Ihren Account fest.</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-black/60">Passwort</label>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? "Speichern…" : "Passwort festlegen & einloggen"}
            </button>
          </form>
        </div>
      </div>
